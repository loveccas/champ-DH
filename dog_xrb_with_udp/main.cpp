...
#define UDP_TARGET_IP "172.21.19.247"
...
void udp_callback(uint32_t size, uint8_t *buffer, udp::RecvInfo info)
{
  // std::cout << "Invalid part in string: " << std::endl;
  if (info.flag == FLAG_SUCCESS)
  {
    ...
    if (std::stoi(id_) == 0)
    {
      ...
      outFile << "id:  " << cmd.id << " pos:  " << cmd.Pos << " T:  " << cmd.T << std::endl;
      serial.sendRecv(&cmd, &data);
    }
    ...
    {
      cmd.id = 3;
      cmd.Pos = -6.33 * std::strtod(pos_, &endptr) + initial[2];
      // cmd.Pos = initial[2];
      // cmd.T = T_f + tau[2];
      // cmd.Pos = -std::strtod(pos_, &endptr);
      if (*endptr != '\0')
      ...
      // std::cout << "id:  " << cmd.id << " pos:  " << cmd.Pos << " T:  " << cmd.T << std::endl;
      outFile << "id:  " << cmd.id << " pos:  " << cmd.Pos << " T:  " << cmd.T << std::endl;
      serial.sendRecv(&cmd, &data);
    }
    ...
    {
      cmd.id = 7;
      // std::cout << "7: " << std::strtod(pos_, &endptr) << std::endl;
...
bool get_initial_angle()
{
  for (int i = 0; i < 12; i++)
  {
    ...
    if (data.correct)
    {
      cmd.Pos = data.Pos;
      initial[i] = data.Pos;
      std::cout << "id:  " << cmd.id << "initial" << data.Pos << std::endl;
    }
    else
    {
    ...
    //   // cmd.Pos = data.Pos;
    //   initial[0] = data.Pos;
    //   std::cout << "initial" << data.Pos << std::endl;
    // }
    // else
    // {
    //   std::cout << "data is not complete" << std::endl;
